      ...
      <form onSubmit={handleSubmit} className="relative bg-gradient-to-br from-primary/15 via-card/80 to-primary/10 backdrop-blur-xl border-2 border-primary/40 rounded-3xl p-8 md:p-10 space-y-6 shadow-[0_0_60px_rgba(var(--primary-rgb),0.25)] overflow-hidden">
        ...
        <div className="space-y-2">
          <input
            ...
            className="w-full px-5 py-4 bg-background/50 border border-border/50 rounded-xl text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary/50 transition-all text-right"
            dir="rtl"
          />
        </div>
          ...
          <div className="relative flex-shrink-0 mt-1">
            ...
            <div className={`w-5 h-5 rounded border-2 transition-all flex items-center justify-center ${
              consent 
                ...
                : 'bg-background/50 border-border/70 group-hover:border-primary/50'
            }`}>
              {consent && (
        ...
        <button
          type="submit"
          disabled={isLoading}
          className="group relative w-full px-10 py-6 text-xl md:text-2xl font-bold text-primary-foreground rounded-xl overflow-hidden transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] shadow-lg hover:shadow-xl disabled:opacity-70 disabled:cursor-not-allowed"
        >
      ...
      <div className="bg-gradient-to-r from-amber-500/10 via-yellow-500/15 to-amber-500/10 border border-amber-500/30 rounded-2xl p-6 backdrop-blur">
        <div className="flex items-center justify-center gap-1 mb-3">
          {[...Array(5)].map((_, i) => (
            <Star 
              key={i} 
              className="w-6 h-6 fill-amber-400 text-amber-400 drop-shadow-[0_0_8px_rgba(251,191,36,0.5)]" 
            />
          ))}
        </div>
